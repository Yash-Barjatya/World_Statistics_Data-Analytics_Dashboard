...
def label_encoding(data):
    ...
    le = LabelEncoder()
    data_label_encoded[cat_cols] = data_label_encoded[cat_cols].apply(
...
def random_sampling(percentage):
    ...
    sampled_country_list = random.sample(
        country_list, int(len(country_list)*percentage/100))
...
def get_elbow_point(data_coors, n_vals):
    ...
    scalarProduct = np.sum(
        vecFromFirst * npm.repmat(lineVecNorm, n_vals, 1), axis=1)
...
def stratified_sampling(percentage):
    ...
    for name, group in grp:
        mean = group.mean()
        df_group = df_group.append(pd.DataFrame([[name, mean[0], mean[1], mean[2], mean[3], mean[4], mean[5], mean[6], mean[7], mean[8],
                                   mean[9], mean[10], mean[11], mean[12], mean[13], mean[14], mean[15], mean[16]]], columns=col_list), ignore_index=True)
    ...
    for cluster in clusters.values():
        cluster = shuffle(cluster, n_samples=int(
            (len(cluster)*percentage)/100), random_state=0)
    ...
    for i in range(len(temp_df)):
        new_df = new_df.append(
            data_complete[data_complete['Country'] == temp_df[0][i]], ignore_index=True)
...
def compute_pca_loadings(pca):
    df_pca_components = pd.DataFrame(
        pca.components_[0:3].T, columns=["PC1", "PC2", "PC3"])
    pca_loadings = df_pca_components.values * \
        np.sqrt(pca.explained_variance_ratio_[0:3])
    return pca_loadings


def compute_top_k_pca_loadings(pca_loadings, k):
    sum_sqr_pca_loadings = [(i, sum(j**2 for j in pca_loadings[i]))
                            for i in range(len(pca_loadings))]
    sum_sqr_pca_loadings.sort(key=lambda x: x[1])
    top_k_pca_loadings = sum_sqr_pca_loadings[::-1][:k]
    top_k_pca_loadings_names = [original_headers[i[0]]
                                for i in top_k_pca_loadings]
...
def getDimensions():
    ...
    selected_dimensions = compute_top_k_pca_loadings(
        pca_loadings, intrinsic_dim+1)
